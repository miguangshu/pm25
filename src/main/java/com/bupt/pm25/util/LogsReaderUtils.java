...
public class LogsReaderUtils {
     ...
     *
     * @param filePath      文件路径的字符串表示形式
     * @param keyWord1      查找包含某个关键字的信息：非null为带关键字查询；null为全文显示
     * @return      当文件存在时，返回字符串；当文件不存在时，返回null
     */
    public static String readFromFile(String filePath, String keyWord1,String keyWord2){
            ...
            try {
                ...
                while((temp = bufferedReader.readLine()) != null){
                    if(keyWord ==null){
                        ...
                        if(temp.contains(keyWord1)){
                            stringBuffer.append("\n");
                            stringBuffer.append(temp + " ");
                        }
                        if(temp.contains(keyWord2)){
                            stringBuffer.append(temp);
                        }
                    }
                }
